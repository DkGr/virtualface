...
$(document).ready(function() {
    ...
    $("#newpost-content").keyup(function () {
      ...
      if(this.value){
        $("#postPreview").replaceWith('<div style="border: 1px solid #ccc;border-radius: 4px;padding: 6px 12px;" id=postPreview><h5 style="color: #D8D8D8;font-weight: bold;font-style: italic;">Prévisualisation de votre message</h5>'+ findURL(this.value) +'</div>');
      }else{
        $("#postPreview").replaceWith('<div id=postPreview></div>');
      }
    });
...
function loadOlderPosts(){
  ...
  $.ajax({
    type: "GET",
    url: "webservice/olderposts/"+lastPostId,
    complete: function(response) {
      var jsonPosts = JSON.parse(response.responseText);
      var htmlStream = '';
      jsonPosts.forEach(function(entry) {
      ...
      if(htmlStream.length == 0)
      {
          $("#posts-stream").append("<p style=\"text-align:center;\">Il n'y a plus rien à afficher ici</p>");
          endPostStream = true;
      }
      else{
          $("#posts-stream").append(htmlStream);
      }
...
function renderComment(jsonComment, commentpair)
{
    ...
    $.ajax({
        ...
        complete: function(response) {
            ...
            if(!anonymous)
            {
              commentLikeLink = '<a href="javascript:void(0)" onclick="sendNewCommentLike(\''+jsonComment['_id']['$id']+'\');">J\'aime</a>';
            }
            if(hasLiked)
            {
            ...
            var d = new Date();
            var content = '';
            if(jsonComment['content'].startsWith('-----BEGIN PGP MESSAGE-----'))
            {
              content = jsonComment['content'];
            }
            else
            {
              content = findURL(jsonComment['content']);
            }
...
function renderPost(jsonPost)
{
    ...
    $.ajax({
        ...
        complete: function(response) {
            ...
            var authorActionButton = '<!-- Split button -->'+
            '<div style="display:table-cell;vertical-align: top;" class="btn-group">'+
              '<button style="padding: 0px 4px;float:right;" type="button" class="btn btn-info dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">'+
                '<span class="caret"></span>'+
                '<span class="sr-only">Toggle Dropdown</span>'+
              '</button>'+
              '<ul style="top: 0;left: -165px;" class="dropdown-menu">'+
                '<li><a href="postview.php?postid='+jsonPost['_id']['$id']+'">Lien de la publication</a></li>'+
              '</ul>'+
            '</div>';
            if(author.hasOwnProperty('private_key'))
            {
            ...
            if(!anonymous)
            {
              postLikeLink = '<a href="javascript:void(0)" onclick="sendNewPostLike(\''+jsonPost['_id']['$id']+'\');">J\'aime</a>';
            }
            if(hasLiked)
            {
            ...
            var content = '';
            if(postContent.startsWith('-----BEGIN PGP MESSAGE-----'))
            {
              content = postContent;
            }
            else
            {
              content = findURL(postContent);
            }
            var d = new Date();
            htmlPost = '<div id="'+jsonPost['_id']['$id']+'" style="box-shadow: 5px 5px 5px rgba(0, 0, 0, 0.2);" class="panel panel-default">'+
            ...
            if(!anonymous)
            {
              htmlPost += '<div style="border: none; margin-bottom: 0px;box-shadow: 5px 5px 5px rgba(0, 0, 0, 0.2);" class="panel panel-default">'+
...
function loadIdentity()
{
    ...
    $.ajax({
        ...
        complete: function(response) {
            ...
            document.title = "VirtualID - Profil de "+displayname;
...
var __urlRegex = /(\b(https?|ftp):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig;
var __imgRegex = /\.(?:jpe?g|gif|png)$/i;
var __videoRegex = /\.(?:avi|mp4|mov)$/i;

function findURL(text){

    var exp = __urlRegex;
    var youtubeURL = "://www.youtube.com/watch";
    var dailymotionURL = "://www.dailymotion.com/video/";

    return text.replace(exp,function(match){
            __imgRegex.lastIndex=0;
            __videoRegex.lastIndex=0;
            if(__imgRegex.test(match)){
                return '<br/><div style="margin-top: 15px;"><img src="'+match+'" width="250" /></div>';
            }
            else if(__videoRegex.test(match)){
                return '<br/><div style="margin-top: 15px;"><video controls src="'+match+'" width="480"></video></div>';
            }
            else{
                if(text.includes(youtubeURL))
                {
                  var start = text.search(youtubeURL);
                  var videoID = text.substring(youtubeURL.length+start+3, text.length);
                  return '<br/><div style="margin-top: 15px;"><iframe width="560" height="315" src="//www.youtube.com/embed/'+ videoID +'" frameborder="0" allowfullscreen></iframe></div>';
                }
                else if(text.includes(dailymotionURL))
                {
                  var start = text.search(dailymotionURL);
                  var videoID = text.substring(start+dailymotionURL.length, start+dailymotionURL.length + 7);
                  return '<br/><div style="margin-top: 15px;"><iframe frameborder="0" width="480" height="270" src="//www.dailymotion.com/embed/video/'+ videoID +'" allowfullscreen></iframe></div>';
                }
                else{
                  var rtnHtml = 'test';
                  $.ajax({
                      async: false,
                      type: "GET",
                      url: "webservice/extracturl",
                      data: 'url='+match,
                      complete: function(response) {
                        var jsonResp = JSON.parse(response.responseText);
                        var MetaDescription = jsonResp.desc;
                        var Title = jsonResp.title;
                        rtnHtml = '<div style="margin:20px;" class="panel panel-default"><div style="font-weight: bold;" class="panel-heading"><a href="'+match+'">'+Title+'</a></div><div class="panel-body">'+MetaDescription+'</div><div class="panel-footer"><img alt="favicon" src="https://www.google.com/s2/favicons?domain='+match+'"> <a href="'+match+'">'+extractDomain(match)+'</a></div></div>';
                      }
                  });
                  return rtnHtml;
                }
