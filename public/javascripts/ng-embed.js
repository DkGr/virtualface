        ...
        .directive('ngEmbed', ['$filter', '$sce', '$http', '$timeout', '$templateCache', function ($filter, $sce, $http, $timeout, $templateCache) {
            ...
            return {
                ...
                link       : function (scope, elements, attributes) {
                    ...
                    scope.basicLink = {};
                    scope.basicLink.needToEmbed = false;
                    scope.video = {};
                    scope.image = {};
                    scope.pdf = {};
                    scope.audio = {};
                    ...
                    var videoProcess = {
                        ...
                        embed: function (data, options) {
                            ...
                            if (data.match(p)) {
                                scope.unknownEmbedSource = false;
                        ...
                        dailymotionEmbed: function (str, opts) {
                            ...
                            if (matches) {
                                ...
                                var i = 0;
                                while (i < matches.length) {
                                    var frame = $sce.trustAsHtml('<iframe src="//www.dailymotion.com/embed/video/' + matches[i].split('/')[2] + '" height="' + videoDimensions.height + '" width="' + videoDimensions.width + '"></iframe>');
                                    scope.videoServices.push(frame);
                        ...
                        liveleakEmbed: function (str, opts) {
                            ...
                            if (matches) {
                                ...
                                var i = 0;
                                while (i < matches.length) {
                                    var frame = $sce.trustAsHtml('<iframe src="//www.liveleak.com/e/' + matches[i].split('=')[1] + '" height="' + videoDimensions.height + '" width="' + videoDimensions.width + '"></iframe></div>');
                                    scope.videoServices.push(frame);
                                    i++;
                                }
                            }
                    ...
                    var basicLinkProcess = {
                      embed: function (str) {
                        var urlRegex = /\b(?:(https?|ftp|file):\/\/|www\.)[-A-Z0-9+()&@$#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|]/ig;
                        var protocolRegex = /^[a-z]+\:\/\//i;
                        scope.basicLink.needToEmbed = false;
                        var strReplaced = str.replace(urlRegex, function (text) {
                          scope.basicLink.needToEmbed = true;
                          var url = text;
                          $http.get('api/extracturl?url=' + url)
                              .success(function (d) {
                                  scope.basicLink.title = d.title;
                                  scope.basicLink.desc = d.desc;
                                  scope.basicLink.url = d.url;
                                  scope.basicLink.domain = d.domain;
                                  scope.basicLink.image = d.image;
                              });
                        });
                        return str;
                      }
                    ...
                    if(scope.unknownEmbedSource){
                      x = basicLinkProcess.embed(x);
                    }
